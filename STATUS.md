...
- **流程**：主会话实测 3 卡 db text_raw + 背景勘察（关键发现：db 特殊能量 types/provides 均 null → CardDef.energy_type=None，旧 `_energy_satisfied` 只当【无】计数——夜光应为彩虹是错误口径，且提供值无 DSL 声明面）→ TDD 任务书定稿（D-WP8-1~5 + 清单 9 条）→ 子代理实现（机制+3 卡一批）→ 规格复核批准；质量复核因 API 限流改主会话自做（进行中）
- **机制层**：①`provide_energy` 声明式框架（能量卡 passive_static：types=[属性]/all 彩虹；分层求值=有条件块覆盖无条件块、同层 ≥2 条 DslError、零条回退 energy_type 旧口径；`_units_cover_cost` 匹配=有色先精确后彩虹再无色，彩虹全程 1 单元；攻击枚举与执行共用求值点）；②夜光降级条件 `holder_special_energy_count_ge:2`（特殊能量=ENERGY 非 basic 含自身计数——两张夜光互相降级，忠实文本）；③喷射换位：手动附着行动 bench 落点直发新事件 `own_attach_from_hand_to_bench` + switch 新 selector self（效果附着不触发=「从手牌附着」一期口径，附录 A 记）；④薄雾 protection：`_protected_from_attack_effects` 声明来源扩为宝可梦卡 ∪ 附着能量卡并集（「已经受到的效果不会消失」=落点守卫天然满足）；⑤Agent 侧三处 `_energy_satisfied` 保持自由函数旧口径（D-WP8-5 已知近似，合法性由引擎枚举门控）
- **新卡 3 张（闸 1/2 过，first_pass 3/3，gate3 待核销）**：喷射能量（5 印刷全挂）/ 夜光能量（池内文本类单挂 CSV1C-127；另 7 印刷「还附着了」异文本类按严格拆分未落地）/ 薄雾能量（3 印刷全挂）；分片 `tests/test_dsl_cards_b8_wp8.py` 6 用例 + `tests/test_primitives_wp8.py` 11 用例
- **规格复核批准**：D-WP8-1~5 逐条符合；3 卡对 db text_raw 忠实、夜光异文本拆分正确；822 绿 + ruff 零告警 + dsl-check 86 文件 OK + 镜像 100 局串/并行 hash 一致；附带验证 多龙喷火龙 vs 喷火龙大比鸟 20 局 0 失败
- **过程纪要**：当日下午 API 反复 429（网关限流，与 403 配额不同——平台过载 + 高频子代理并发叠加）；用户裁决降本：小批次复核主会话自做、子代理串行不并行、双复核可合并、限流期优先零 LLM 成本工作（冒烟/落账/提交）
- **质量复核（主会话自做）**：core.py/chooser.py/primitives.py/vocabularies/测试/3 卡 YAML diff 全量审读——匹配算法贪心证明正确（精确单元只匹配本属性，精确优先不会劣化）、switch self 状态清除方向合 §7.1、夜光降级断言双向覆盖、求值点一致性含同种子重放；**无必修项批准**
- **真机冒烟 80 局 0 失败**（heuristic 4×20，`results/wp8-smoke/`）：喷火龙大比鸟 vs 赫普的苍响 6/14；多龙喷火龙 vs 喷火龙大比鸟 7/13；多龙巴鲁托 vs 赫普的苍响 6/14；多龙喷火龙 vs 多龙黑夜魔灵 9/11；机制真实触发——own_attach_from_hand_to_bench 3/6 次（喷射换位真实发生）、switch 43–85 次/库、protected 10–57 次/库
- **测试**：WP8 新 17 条；**全量 822 绿（基线 805）+ ruff 零告警 + dsl-check --db 全库 86 文件全 OK + 镜像 hash 回归绿**
- **落账**：coverage-plan 3 行 pending→done——**缺口 done 66 / blocked 0 / pending 15（共 81，余 C 级：task 027 域 7 + task 029 域 8）**；authoring-log 批 7 三条；附录 A D-WP8-1~5 共 5 条 🔲 待核；PRD §5.1 WP8 段
- **遗留**：3 个卡文件 gate3 待用户核销 + 附录 A 5 条决议 🔲 待核（连同 WP6/WP7 攒批）；夜光能量异文本类 7 印刷未落地（回归池内需另文新写）；下一步 task 027（ACE SPEC + TERA 规则盒核对 + 7 卡，需立项）→ task 029（持续 lock/protection 体系 + 8 卡，需立项）
